...
export class NewsfeedBoostRotator {
  ...
  load() {
    return new Promise((resolve, reject) => {
      ...
      this.client
        .get('api/v1/boost/fetch/newsfeed', { limit: 10, rating: this.rating })
        .then((response: any) => {
          if (!response.boosts) {
  ...
  setRating(rating) {
    ...
    this.load().then(() => {
      this.client.post(
        'api/v1/settings/' + this.session.getLoggedInUser().guid,
        {
          boost_rating: rating
        }
      );
  ...
  recordImpression(position: number, force: boolean) {
    ...
    if (
      ...
      this.client
        .put(
          'api/v1/boost/fetch/newsfeed/' + this.boosts[position].boosted_guid
        )
        .then(() => null)
        .catch(() => null);
    }
