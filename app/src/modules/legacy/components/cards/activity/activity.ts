...
export class Activity {
  ...
  @Input()
  set boostToggle(toggle: boolean) {
    //if(toggle)
    //  this.showBoost();
  }
  ...
  save() {
    ...
    this.client
      .post('api/v1/newsfeed/' + this.activity.guid, this.activity)
  ...
  mute() {
    ...
    this.client
      .post(`api/v1/entities/notifications/${this.activity.guid}/mute`)
      .then((response: any) => {
        if (response.done) {
          this.activity['is:muted'] = true;
          return;
  ...
  unmute() {
    ...
    this.client
      .post(`api/v1/entities/notifications/${this.activity.guid}/unmute`)
      .then((response: any) => {
        if (response.done) {
          this.activity['is:muted'] = false;
          return;
  ...
  asyncMuteFetch() {
    ...
    this.client
      .get(`api/v1/entities/notifications/${this.activity.guid}`)
      .then((response: any) => {
  ...
  showWire() {
    this.overlayModal.create(WireCreatorComponent, this.activity).present();
  }

  feature() {
    this.activity.featured = true;
    this.client.put('api/v1/admin/feature/' + this.activity.guid).catch(() => {
  ...
  unFeature() {
    this.activity.featured = false;
    this.client
      .delete('api/v1/admin/feature/' + this.activity.guid)
      .catch(() => {
        this.activity.featured = true;
      });
  }
  ...
  setExplicit(value: boolean) {
    ...
    this.client
      .post(`api/v1/admin/activity/${this.activity.guid}/mature`, {
        value: value ? '1' : '0'
      })
      .catch(e => {
  ...
  isVisible() {
    ...
    this.scroll_listener = this.scroll.listenForView().subscribe(view => {
      if (
        this.element.offsetTop - this.scroll.view.clientHeight <=
