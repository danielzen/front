...
export class Channel {
  ...
  load() {
    ...
    this.client
      ...
      .then((data: MindsChannelResponse) => {
        ...
        if (this._filter === 'feed') this.loadFeed(true);
      })
      .catch(e => {
        if (e.status === 0) {
  ...
  loadFeed(refresh: boolean = false) {
    ...
    this.client
      .get(
        'api/v1/newsfeed/personal/' + this.user.guid,
      ...
      .then((data: MindsActivityObject) => {
        ...
        if (this.feed && !refresh) {
          for (let activity of data.activity) this.feed.push(activity);
        } else {
  ...
  update() {
    this.client.post('api/v1/channel/info', this.user).then((data: any) => {
      this.editing = false;
      if (this.editForward) {
  ...
  delete(activity) {
    let i: any;
    for (i in this.feed) {
      if (this.feed[i] === activity) this.feed.splice(i, 1);
    }
  }
