...
export class ThumbnailSelector {
  ...
  ngOnInit() {
    ...
    this.element.addEventListener('loadedmetadata', () => {
      if (this.thumbnailSec) this.element.currentTime = this.thumbnailSec;
  ...
  set _thumbnailSec(value: number) {
    if (!this.canvas) this.inProgress = true;
  ...
  createThumbnail() {
    ...
    this.element.addEventListener('seeked', () => {
      ...
      this.canvas
        .getContext('2d')
        .drawImage(this.element, 0, 0, this.canvas.width, this.canvas.height);
      this.thumbnail.next([
