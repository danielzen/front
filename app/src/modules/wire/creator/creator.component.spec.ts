...
fdescribe('WireCreatorComponent', () => {
  ...
  it('should have subtext', () => {
    ...
    expect(subtitle.nativeElement.textContent).toContain(
      'Support @' +
      comp.owner.username +
      ' by sending them dollars, points or Bitcoin. Once you send them the amount listed in the tiers, ' +
      'you can receive rewards if they are offered. Otherwise, it\'s a donation.'
    );
  });
  ...
  it('should have payment method list (points, usd and bitcoin)', () => {
    ...
    expect(
      fixture.debugElement.query(
        By.css(
          '.m-wire--creator-selector > li:first-child > .m-wire--creator-selector-type > h4'
        )
    ...
    expect(
      fixture.debugElement.query(
        By.css(
          '.m-wire--creator-selector > li:nth-child(2) > .m-wire--creator-selector-type > h4'
        )
      ).nativeElement.textContent
    ).toContain('USD');
    expect(
      fixture.debugElement.query(
        By.css(
          '.m-wire--creator-selector > li:last-child > .m-wire--creator-selector-type > h4'
        )
      ).nativeElement.textContent
    ).toContain('Bitcoin');
  });
  ...
  it(
    'usd payment option should only be available if the user\'s a merchant',
    fakeAsync(() => {
      ...
      expect(
        moneyOption.nativeElement.classList.contains(
          'm-wire--creator-selector--disabled'
        )
      ).toBeTruthy();
    })
  );
  ...
  it(
    'if selected payment method is usd, then a card selector should appear',
    fakeAsync(() => {
      tick();
      ...
      expect(
        fixture.debugElement.query(By.css('.m-wire--creator-payment'))
      ).not.toBeNull();
    })
  );
  ...
  it(
    'if there are any errors, hovering over the submit section should show them',
    fakeAsync(() => {
      ...
      expect(getErrorLabel().nativeElement.textContent).toContain(
        'I\'m an error'
      );
    })
  );
  ...
  it('send button should be disabled either if the user hasn\'t entered data, there\'s an error, ' +
    'the component\'s loading something or just saved the wire', () => {
  ...
  it(
    ...
    fakeAsync(() => {
      ...
      clientMock.response[`api/v1/wire/${comp.wire.guid}`] = {
        status: 'success'
      };
