...
@Component({
  ...
  template: `
    ...
    <minds-checkout-card-input (confirm)="setCard($event)" [hidden]="inProgress || confirmation"
                               *ngIf="useCreditCard"></minds-checkout-card-input>
...
export class Checkout {
  ...
  init() {
    System.import('lib/braintree.js').then((braintree: any) => {
      this.client
        .get('api/v1/payments/braintree/token/' + this.gateway)
        .then((response: any) => {
          this.setupClient(braintree, response.token);
        });
    });
  }
  ...
  purchase() {
    ...
    this.error = '';
    this.client
      .post('api/v1/payments/braintree/charge', {
        nonce: this.nonce,
        amount: this.amount,
