...
@Component({
  ...
  inputs: ['user'],
  outputs: ['userChanged'],
  template: `<button class="material-icons" (click)="toggleMenu($event)">settings</button>
...
<ul class="minds-dropdown-menu" [hidden]="!showMenu">
  ...
  <li class="mdl-menu__item" *ngIf="session.isAdmin()" [hidden]="user.banned === 'yes'" (click)="banToggle = true; showMenu = false" i18n>
    Ban globally
  </li>
  <li class="mdl-menu__item" *ngIf="session.isAdmin()" [hidden]="user.banned !== 'yes'" (click)="unBan()" i18n>Un-ban globally</li>
  <li class="mdl-menu__item" *ngIf="session.isAdmin()" [hidden]="user.ban_monetization === 'yes'"
      (click)="banMonetizationToggle = true; showMenu = false" i18n>Ban from Monetization
  </li>
  <li class="mdl-menu__item" *ngIf="session.isAdmin()" [hidden]="user.ban_monetization !== 'yes'" (click)="unBanMonetization()" i18n>
...
export class UserDropdownButton {
  ...
  ban() {
    ...
    this.client
      .put(`api/v1/admin/ban/${this.user.guid}`, {})
      .then(() => {
        this.user.banned = 'yes';
      })
      .catch(e => {
