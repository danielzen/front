...
@Component({
  ...
  outputs: ['added'],
  template: `<div class="minds-banner" *ngIf="!editing">
  <div class="minds-banner-img mdl-color--blue-grey"
       [ngStyle]="{'background-position': '0 ' + top + 'px', 'background-image': 'url(' + src + ')'}"></div>
  <div class="minds-banner-overlay"></div>
</div>
<div *ngIf="editing" class="minds-banner minds-banner-editing">
  <img class="mdl-color--blue-grey" src="{{src}}" [ngStyle]="{'top': top}" (dragstart)="dragstart($event)" (dragover)="drag($event)"
