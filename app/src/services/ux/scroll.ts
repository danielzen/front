...
export class ScrollService {
  ...
  listen(callback: Function, debounce: number = 0, throttle: number = 0): any {
    if (debounce) {
      return this.scroll.debounceTime(debounce).subscribe(callback);
    }
    if (throttle) {
      return this.scroll.throttleTime(throttle).subscribe(callback);
    }
  ...
  listenForView() {
    if (!this.viewListener) {
      this.viewListener = this.scroll.debounceTime(500).subscribe(e => {
