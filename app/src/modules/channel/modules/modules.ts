...
@Component({
  ...
  template: `<div class="mdl-card__title" [routerLink]="linksTo">
...
export class ChannelModulesComponent {
  ...
  load() {
    ...
    this.client
      ...
      .then((response: any) => {
        if (!(response.entities || response.blogs)) return false;

        if (this.type === 'blog') this.items = response.blogs;
        else this.items = response.entities;
