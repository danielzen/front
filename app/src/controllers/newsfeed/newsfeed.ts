...
export class Newsfeed {
  ...
  pollingLoadNew() {
    ...
    if (this.pollingNewPosts > 120) {
      // just replace the whole newsfeed
  ...
  autoBoost(activity: any) {
    this.client.post(
      'api/v1/boost/activity/' + activity.guid + '/' + activity.owner_guid,
      {
        newUserPromo: true,
        impressions: 200,
  ...
  delete(activity) {
    ...
    for (i in this.newsfeed) {
      if (this.newsfeed[i] === activity) this.newsfeed.splice(i, 1);
    }
  }

  @HostListener('window:resize')
  detectWidth() {
    if (window.innerWidth < 1200) this.showRightSidebar = false;
    else this.showRightSidebar = true;
  }
