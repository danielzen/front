...
@Component({
  ...
  template: `<m-modal [open]="open" (closed)="onClose($event)" *ngIf="!session.isLoggedIn() || display != 'initial'">
  ...
  <div *ngIf="display == 'initial'">
    ...
    <div class="m-signup-modal-feature-text mdl-card__supporting-text">
      <!-- i18n -->An open-source, community-owned <b>social network</b> providing tools for revenue, reach, privacy and Internet freedom.
      <!-- /i18n -->
    </div>
  ...
  <!-- Login Display -->
  <minds-form-login (done)="done('login')" (doneRegistered)="display = 'fb-complete'" (canceled)="close()"
                    *ngIf="display == 'login'"></minds-form-login>
  <!-- Register Display -->
  <span style="font-weight: bold;text-align:center;font-size: 13px;margin-bottom: -14px;cursor: pointer;" class="mdl-color-text--blue-grey"
        *ngIf="display == 'register'" (click)="do('login')" i18n>Already have a channel? Click here.</span>
  <minds-form-register (done)="done('register')" (canceled)="close()" *ngIf="display == 'register'">
  </minds-form-register>
  <!-- FB Signin final phase -->
  <minds-form-fb-register (done)="done('register')" (canceled)="close()" *ngIf="display == 'fb-complete'">
  </minds-form-fb-register>
  <!-- Categories selector -->
  <minds-onboarding-categories-selector (done)="done('categories')" *ngIf="display == 'categories'">
    ...
    switch (display) {
      case 'login':
        ...
        AnalyticsService.send('pageview', {
          page: this.route + `${op}modal=login`
        });
        this.display = 'login';
        break;
      case 'register':
        ...
        AnalyticsService.send('pageview', {
          page: this.route + `${op}modal=register`
        });
